...
def train_stage2(args, labeled_trainloader, unlabeled_trainloader, val_loader, test_loader, 
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.filter_every_epoch == 0:
            in_dist_idxs = filter_ood(args, udst_eval_loader, model)
        ...
        for batch_idx in range(args.eval_step):
            ...
            feats_x ,feats_x_aug = feats[batch_size:].chunk(2)
            ...
            delta = F.mse_loss(proj, proj_aug, reduction='none').mean(dim=1)
            delta_hat = (delta - delta.min()) / (delta.max() - delta.min() + 1e-8)
            weights = 1 - delta_hat
...
def filter_ood(args, loader, model,cons_proj):
    """
    use representation consistency to filter OOD samples
    """
