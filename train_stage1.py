...
def main():
    parser = argparse.ArgumentParser(description=' Stage1 Training with Mismatch Ratio')
    ...
    def create_model(args):
        ...
        return model, rotnet_head, cons_proj, rot_proj
    ...
    model, rotnet_head, cons_proj, rot_proj = create_model(args)
    model, rotnet_head, cons_proj, rot_proj = model.to(args.device), rotnet_head.to(args.device),\
                                   cons_proj.to(args.device), rot_proj.to(args.device)
    ...
    train_stage1(args, labeled_trainloader, udst_rotnet_loader, val_loader, test_loader,
                 model, rotnet_head, cons_proj, rot_proj, ema_model)
...
def train_stage1(args, labeled_trainloader, unlabeled_trainloader, val_loader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            try:
                inputs_u_w, gt_u, index_u = next(unlabeled_iter)
            except:
                unlabeled_iter = iter(unlabeled_trainloader)
                inputs_u_w, gt_u, index_u = next(unlabeled_iter)
            ...
            _, feats_x_aug = model(augment_input(inputs_x), output_feats=True)
            proj = cons_proj(feats_x)
            proj_aug = cons_proj(feats_x_aug)
            Lcon = F.mse_loss(proj, proj_aug)
          ...
          #  Cross Entropy Loss for Rotation Recognition
            ...
            if not args.no_progress:
                p_bar.set_description("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Data: {data:.3f}s. Batch: {bt:.3f}s. "
                                      "Loss: {loss:.4f}. Loss_x: {loss_x:.4f}. Loss_r: {loss_r:.4f} Loss_con:{loss_con:.4f}.".format(
                    epoch=epoch + 1,
                    epochs=args.epochs,
                    batch=batch_idx + 1,    
                    iter=args.eval_step,
        ...
        args.writer.add_scalar('train/4.train_loss_con', losses_con.avg, epoch)
